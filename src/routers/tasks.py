...
from src.schemas.tasks import TaskSubmitRequest, TaskResponse, TaskListResponse, TaskMessageResponse
...
async def submit_task(request: TaskSubmitRequest, queue: RedisQueue = Depends(get_queue)):
    """Submit a new task to the queue."""
    task_id = await queue.enqueue(request.payload)
    task_data = await queue.get_task(task_id)
    if not task_data:
        raise HTTPException(status_code=500, detail="Task was enqueued but could not be retrieved.")
    return task_data

@router.get("/", response_model=TaskListResponse)
async def list_tasks(limit: int = 50, offset: int = 0, queue: RedisQueue = Depends(get_queue)):
    """List all tasks."""
...
async def cancel_task(task_id: UUID, queue: RedisQueue = Depends(get_queue)):
    ...
    if not success:
        raise HTTPException(status_code=400, detail="Task could not be cancelled. It may not exist or is no longer PENDING.")
...
async def retry_task(task_id: UUID, queue: RedisQueue = Depends(get_queue)):
    ...
    if not success:
        raise HTTPException(status_code=400, detail="Task could not be retried. It may not exist or is not FAILED.")
