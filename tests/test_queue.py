...
async def test_enqueue_and_dequeue(queue):
    ...
    task_id = await queue.enqueue("test_task", payload)
...
async def test_peek(queue):
    ...
    task_id = await queue.enqueue("test_peek", payload)
...
async def test_cancel_task(queue):
    task_id = await queue.enqueue("cancel_task", {"data": "cancel_me"})
...
async def test_fail_task_with_retry(queue):
    task_id = await queue.enqueue("fail_task", {"data": "fail_me"})
...
async def test_fail_task_max_retries(queue):
    task_id = await queue.enqueue("fail_task_max", {"data": "fail_me_max"})
    await queue.dequeue()
...
async def test_retry_task(queue):
    task_id = await queue.enqueue("retry_task", {"data": "manual_retry"})
    await queue.dequeue()
    await queue.fail_task(task_id, "Test error", max_retries=0, base_delay=0)
...
async def test_list_tasks_pagination(queue):
    ...
    for i in range(5):
        await queue.enqueue(f"task_{i}", {"task": i})
